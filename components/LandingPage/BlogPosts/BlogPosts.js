...
const BlogPosts = ({ articles }) => (
	<div css={tw`block md:flex`}>
		<div css={tw`text-center px-3 md:px-6 text-light py-4 md:w-1/2 md:text-left`}>
			<h1
				css={tw`font-bold text-white whitespace-no-wrap text-3xl sm:text-4xl md:text-5xl lg:text-6xl xl:text-huge`}
			>
				Latest Posts
			</h1>
			<p css={tw`text-white text-sm md:text-base lg:text-lg`}>
				I also write articles in my free time. I write about new technologies that I find
				interesting, solutions to an issue that bugs me, and some snippets that I hope
				someone will find useful. My posts are about web development and on that mostly
